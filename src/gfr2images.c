...
static void drawTranscript (int numberExons, int yCoordinateTranscript, gdImagePtr im, int color)
{
  ...
  xEndCoordinateTranscript = SIDE_MARGIN + numberExons * EXON_SIZE + (numberExons + 1) * SPACER;
  gdImageLine(im,SIDE_MARGIN,yCoordinateTranscript,xEndCoordinateTranscript,yCoordinateTranscript,color);
...
void drawNumbers(gdImagePtr im, int numExons, int color) {
  ...
  for( i=1; i<=numExons; i++) {
    ...
    gdImageString( im, gdFontGetTiny(), 
...
static void makeImage (GfrEntry *currGE)
{
  ...
  imageWidth = MAX (maxNumberExons * EXON_SIZE + (maxNumberExons + 1) * SPACER + 2 * SIDE_MARGIN,MIN_IMAGE_WIDTH);
